...
public class ToppingSpawnManager : MonoBehaviour
{
    ...
    //        log("tsm : " + nowToppingObject.name + " 복사.");
    //    }
    //    log("tsm: 타르트에 필요한 토핑 스폰 완료. 70% 스폰 시작");

    //    toppingBonusCount = (int)toppingCount / 7; // 보너스 토핑의 개수를 정함.
    ...
    //        bool isPossibleTopping = false;

    //        for (int i = 0; i < bonusCheck.Length; i++)
    //        {
    //            if (bonusCheck[i] == bonIndexTemp)//같으면
    //            {
    //                continue;
    //            }
    //            else//다르면 (사용 가능)
